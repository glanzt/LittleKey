...
export default function RegisterPage() {
  ...
  async function handleRegister(e) {
    ...
    if (!res.ok) {
      ...
      setError(messages[data.error] || "ההרשמה נכשלה, נסי שוב");
      return;
    }
    ...
    if (result?.error) {
      setError("ההרשמה הצליחה, אך ההתחברות נכשלה. נסי להתחבר.");
      return;
    }
  ...
  return (
    <div style={{
      ...
      <p style={{ color: "#7F8C8D", marginBottom: "2rem", fontSize: "1.1rem" }}>צרי חשבון כדי לשמור את ההתקדמות</p>
      ...
      <div style={{
        ...
        <form onSubmit={handleRegister}>
          ...
          <input type="email" placeholder="אימייל" value={email} onChange={(e) => setEmail(e.target.value)} required style={inputStyle} />
          <input type="password" placeholder="סיסמה (לפחות 6 תווים)" value={password} onChange={(e) => setPassword(e.target.value)} required style={inputStyle} />
          <input type="password" placeholder="אימות סיסמה" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required style={inputStyle} />
          ...
          <button type="submit" disabled={loading} style={{
            ...
            background: "linear-gradient(135deg, #7C5CFC, #5B3FD4)", color: "white",
            fontSize: "1.1rem", fontFamily: "'Secular One', sans-serif",
            cursor: loading ? "wait" : "pointer", opacity: loading ? 0.7 : 1,
            boxShadow: "0 4px 16px rgba(124,92,252,0.3)", transition: "opacity 0.2s",
          }}>
        ...
        <p style={{ textAlign: "center", marginTop: "1.2rem", fontSize: "0.9rem", color: "#999" }}>
          כבר יש לך חשבון?{" "}
          <a href="/auth/signin" style={{ color: "#E74C3C", textDecoration: "none", fontWeight: 600 }}>
            התחברי
          </a>
        </p>
      </div>
